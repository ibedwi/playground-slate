import { Button, Container, Heading, Select, Stack, Text } from "@chakra-ui/react";
...
import { ButtonElement, CustomElement, } from "@modules/slate/slateEntity";
import { useCallback, useEffect, useMemo, useState } from "react";
import { createEditor, Descendant, Editor, Element, Node, Transforms } from "slate";
import { Slate, Editable, withReact, RenderElementProps, RenderLeafProps, useSlate, useFocused, useSlateSelection, useSlateStatic } from "slate-react";
  ...
  return (
    ...
    >
      <Stack direction={"row"} h={"100vh"} overflow="hidden">
        ...
        <Stack borderLeftWidth={1} borderColor={"gray.300"} overflow="auto" w={600} justifyContent="space-between">
    ...
    <Stack px={2} py={2}>
      <Heading fontSize={"2xl"}>Element Editor</Heading>
      {
        ...
        <>
          <Stack px={2}>
            <Select
              onChange={(e) => changeHeadingLevel(Number(e.target.value) as 1 | 2)}
...

      {
        ...
        <Stack px={2}>
          <Select
            onChange={(e) => changeButtonVariant(e.target.value as "solid" | "outline" | "ghost")}
